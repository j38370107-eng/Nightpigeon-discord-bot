...
export const seenCmd: Command = {
  ...
  async execute(message: Message, args: string[], _client: Client) {
    ...
    if (!entry) {
      return void message.reply(`❌ No data for **${target.user.tag}** — they may not have sent a message since the bot was added.`);
    }
...
export const cleanupCmd: Command = {
  ...
  async execute(message: Message, args: string[], _client: Client) {
    ...
    if (toDelete.length === 0) {
      const n = await channel.send("❌ No recent messages found for that user (messages may be older than 14 days).");
    ...
    const notice = await channel.send(`🗑️ Deleted **${deleted}** message${deleted !== 1 ? "s" : ""} from **${target.user.tag}**.`);
